...
describe('Review e2e test', () => {
  ...
  it('should create and save Reviews', async () => {
    async function createReview() {
      await reviewComponentsPage.clickOnCreateButton();
      await reviewUpdatePage.setNumberInput('number');
      expect(await reviewUpdatePage.getNumberInput()).to.match(/number/);
  ...
  it('should delete last Review', async () => {
    ...
    expect(await reviewDeleteDialog.getDialogTitle().getAttribute('id')).to.match(/studentmanagementsystemApp.review.delete.question/);
